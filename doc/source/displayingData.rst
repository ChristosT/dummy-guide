...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/MultipleViewsInParaView.png}
\caption{Using multiple views in |paraview| to generate different
types of visualizations from a dataset.}
.. _fig:MultipleViewsInParaView:
\end{center}
\end{figure}
...
Most of the scientific datasets discussed in Section :ref:`sec:VTKDataModel`
...
(Section :ref:`sec:VTKDataModel:Table`), you cannot directly show that data in
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/RenderViewInParaView.png}
\caption{|paraview| using  ``Render View`` :index:`\ <Render View>`\  to generate 3D
visualizations from a dataset.}
.. _fig:RenderViewInParaView:
\end{center}
\end{figure}
...
* *Surface rendering* methods provide general rendering by rendering a surface mesh for the dataset. For polygonal datasets (Section :ref:`sec:VTKDataModel:PolyData`), this is simply the raw data. In cases of other datasets including structured (Sections :ref:`sec:VTKDataModel:UniformRectilinearGrid`, \ref{sec:VTKDataModel:RectilinearGrid}, \ref{sec:VTKDataModel:CurvilinearGrid}) and unstructured (Section :ref:`sec:VTKDataModel:UnstructuredGrid`) grids, this implies extracting a surface mesh for all external faces in the dataset and then rendering that mesh. The surface mesh itself can then be rendered as a filled surface or as a wireframe simply showing the edges, etc.

* *Slice* rendering is available for uniform rectilinear grid datasets (Section :ref:`sec:VTKDataModel:UniformRectilinearGrid`) where the visualization is generated by simply rendering an orthogonal slice through the dataset. The slice position and alignment can be selected using the display properties.
...
in the view (Figure  :ref:`fig:RenderViewPropertiesPanel`).

\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.4\linewidth]{Images/RenderViewPropertiesPanel.png}
\caption{The  ``Properties`` :index:`\ <Properties>`\  panel showing view properties for  ``Render
  View`` :index:`\ <Render
  View>`\ .}
.. _fig:RenderViewPropertiesPanel:
\end{center}
\end{figure}

 ``Axes Grid`` :index:`\ <Axes Grid>`\  refers to an annotation axis rendered around all
datasets in the view (Figure :ref:`fig:RenderViewAxesGrid`). You use
...
described in Chapter :ref:`chapter:AxesGrid`.

\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/RenderViewAxesGrid.png}
\caption{ ``Axes Grid`` :index:`\ <Axes Grid>`\  is used to annotate data bounds in  ``Render View`` :index:`\ <Render View>`\ .}
.. _fig:RenderViewAxesGrid:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/WaveletRepresentations.png}
\caption{
  Different renderings generated by rendering data produced by the  ``Wavelet`` :index:`\ <Wavelet>`\ 
  source as outline, points, slice, surface, surface with edges, and volume.}
.. _fig:WaveletRepresentations:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/UnderstandingMapScalars.png}
\caption{The  ``Map Scalars`` :index:`\ <Map Scalars>`\  property can be used to avoid using a transfer
  function and directly interpreting the array values as colors, if possible.}
.. _fig:UnderstandingMapScalars:
\end{center}
\end{figure}

\begin{center}
\includegraphics[width=0.5\linewidth]{Images/PolarAxesCheckbox.png}
\end{center}
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/PolarAxesEdit.png}
\end{center}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/PolarAxes.png}
\caption{A  ``Polar Axes`` :index:`\ <Polar Axes>`\  usage example.}
.. _fig:PolarAxes:
\end{center}
\end{figure}

\begin{center}
\includegraphics[width=0.5\linewidth]{Images/RenderViewDisplayPropertiesStylingGroup.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/RenderViewDisplayPropertiesLightingGroup.png}
\end{center}
...
.. admonition:: **Common Errors** 
  ...
  \begin{center}
  \includegraphics[width=0.7\linewidth]{Images/ScalarColoringAndSpecularHighlights.png}
  \end{center}

\begin{center}
\includegraphics[width=0.5\linewidth]{Images/RenderViewDisplayPropertiesEdgeStylingGroup.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/RenderViewDisplayPropertiesBackfaceStylingGroup.png}
\end{center}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/NonLinearSubdivision.png}
\caption{A dataset made of quadratic tetra hedra displayed with 1, 2, and 3 levels of nonlinear
subdivision.}
.. _fig:NonLinearSubdivision:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/LineChartViewInParaView.png}
\caption{|paraview| using  ``Line Chart View`` :index:`\ <Line Chart View>`\  to plot data values
probed along a line through the dataset using  ``Plot Over Line`` :index:`\ <Plot Over Line>`\  filter.}
.. _fig:LineChartViewInParaView:
\end{center}
\end{figure}
...
(Figure :ref:`fig:LineChartViewUsingLogScale`). Note that log
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/LineChartViewUsingLogScale.png}
\caption{Differences between line charts when using log scale for the Y-axis.}
.. _fig:LineChartViewUsingLogScale:
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/LineChartViewDisplayPropertiesAttributeType.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\linewidth]{Images/LineChartViewDisplayPropertiesSeriesParametersGroup.png}
\end{center}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/BarChartViewInParaView.png}
\caption{|paraview| using  ``Bar Chart View`` :index:`\ <Bar Chart View>`\  to plot the
histogram for a dataset using the  ``Histogram`` :index:`\ <Histogram>`\  filter.}
.. _fig:BarChartViewInParaView:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/BoxChartViewInParaView.png}
\caption{|paraview| using  ``Box Chart View`` :index:`\ <Box Chart View>`\  to plot the
box chart for a dataset using the  ``Compute Quartiles`` :index:`\ <Compute Quartiles>`\  filter.}
.. _fig:BoxChartViewInParaView:
\end{center}
\end{figure}
...
\caption{|paraview| using  ``Plot Matrix View`` :index:`\ <Plot Matrix View>`\  to generate a
scatter plot matrix to understand correlations between pairs of variables.}
.. _fig:PlotMatrixViewInParaView:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/ParallelCoordinatesViewInParaView.png}
\caption{|paraview| using  ``Parallel Coordinates View`` :index:`\ <Parallel Coordinates View>`\  to plot correlations
between columns in a table.}
.. _fig:ParallelCoordinatesViewInParaView:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/ParallelCoordinatesViewInParaViewSelection.png}
\caption{High ``S'' data point are influenced more by low ``Ks'' than high ``Q''}
.. _fig:ParallelCoordinatesViewInParaViewSelection:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/SpreadSheetViewInParaView.png}
\caption{|paraview| using  ``SpreadSheet View`` :index:`\ <SpreadSheet View>`\  to plot raw data values
for the $can.ex2$ dataset.}
.. _fig:SpreadSheetViewInParaView:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/SliceViewInParaView.png}
\caption{ ``Slice View`` :index:`\ <Slice View>`\  can be used to show orthogonal slices from datasets.}
.. _fig:SliceViewInParaView:
\end{center}
\end{figure}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\linewidth]{Images/PythonViewInParaView.png}
\caption{|paraview| using  ``Python View`` :index:`\ <Python View>`\  with matplotlib
to display a scatterplot of momentum magnitude versus density
(upper right) and a histogram of density (lower right) in the
bluntfin.vts dataset.}
.. _fig:PythonViewInParaView:
\end{center}
\end{figure}
...
\item  ``GetNumberOfVisibleDataObjects()`` :index:`\ <GetNumberOfVisibleDataObjects()>`\  -
...
\item  ``GetVisibleDataObjectForSetup(visibleObjectIndex)`` :index:`\ <GetVisibleDataObjectForSetup(visibleObjectIndex)>`\  -
...
\item  ``GetNumberOfAttributeArrays(visibleObjectIndex, attributeType)`` :index:`\ <GetNumberOfAttributeArrays(visibleObjectIndex, attributeType)>`\  -
...
\item  ``GetAttributeArrayName(visibleObjectIndex, attributeType, arrayIndex)`` :index:`\ <GetAttributeArrayName(visibleObjectIndex, attributeType, arrayIndex)>`\  -
...
\item  ``SetAttributeArrayStatus(visibleObjectIndex, vtkDataObject.POINT, "Density", 1)`` :index:`\ <SetAttributeArrayStatus(visibleObjectIndex, vtkDataObject.POINT, "Density", 1)>`\  -
...
\item  ``GetAttributeArrayStatus(visibleObjectIndex, vtkDataObject.POINT, "Density")`` :index:`\ <GetAttributeArrayStatus(visibleObjectIndex, vtkDataObject.POINT, "Density")>`\  -
...
\item  ``EnableAllAttributeArrays()`` :index:`\ <EnableAllAttributeArrays()>`\  -
 This sets all arrays to be copied.

\item  ``DisableAllAttributeArrays()`` :index:`\ <DisableAllAttributeArrays()>`\  -
 This sets all arrays to not be copied.

\end{itemize}

% Some of the text in the \py environment doesn't break to the next
% line and extends into the margin. The ragged right environment
% fixes this, but resets the indentation length to zero, so we have
% to save it here and restored it after the \raggedright section.
...
the  ``setup\_data(view)`` :index:`\ <setup\_data(view)>`\  function.
}
...
After the  ``setup\_data(view)`` :index:`\ <setup\_data(view)>`\  function has been called, |ParaView|
